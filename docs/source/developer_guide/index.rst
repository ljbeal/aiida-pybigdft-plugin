...
``aiida-pybigdft-plugin`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-pybigdft-plugin`` repository (preferably using ``aiida-pybigdft-plugin`` as the project name)

The documentation is now available at `aiida-pybigdft-plugin.readthedocs.io <http://aiida-pybigdft-plugin.readthedocs.io/>`_.
